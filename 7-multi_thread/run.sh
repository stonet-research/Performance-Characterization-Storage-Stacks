...
do
    ...
    do
    numactl -C $((20-${t}))-19 -m 1 env FIO_RUN_TIME=$FIO_RUN_TIME  FIO_RAMP_TIME=$FIO_RAMP_TIME $FIO_EXE ${e}.conf --thread=1 --output-format=json --numjobs=${t} -o $RESULT/${e}_t_${t}.txt;
    done
done
...
do
    ...
    do
    numactl -C $((20-${t}))-19 env FIO_RUN_TIME=$FIO_RUN_TIME  FIO_RAMP_TIME=$FIO_RAMP_TIME $FIO_EXE ${e}.conf --thread=1 --output-format=json --numjobs=${t} -o $RESULT/${e}_t_${t}.txt;
    done
done
...
do
FIO_RUN_TIME=$FIO_RUN_TIME FIO_RAMP_TIME=$FIO_RAMP_TIME $FIO_EXE ${e}.conf --numjobs=${t} --thread=1 --output-format=json -o $RESULT/${e}_t_${t}.txt;
done
...
for t in "${num_threads_socket1[@]}"
do
numactl -C $((20-${t}))-19 -m 1 env FIO_RUN_TIME=$FIO_RUN_TIME FIO_RAMP_TIME=$FIO_RAMP_TIME LD_PRELOAD=$SPDK_FIO_PLUGIN $FIO_EXE ${e}.conf --thread=1 --output-format=json --numjobs=${t} -o $RESULT/${e}_t_${t}.txt;
...
for t in "${num_threads_socket2[@]}"
do
numactl -C $((20-${t}))-19 env FIO_RUN_TIME=$FIO_RUN_TIME FIO_RAMP_TIME=$FIO_RAMP_TIME LD_PRELOAD=$SPDK_FIO_PLUGIN $FIO_EXE ${e}.conf --thread=1 --output-format=json --numjobs=${t} -o $RESULT/${e}_t_${t}.txt;
